...
import requests
from dash import ALL, Input, Output, State, callback_context, dcc, html, callback
...
dash.register_page(
    ...
    path="/transmonee/health",  # this is the default path and working example
    order=1,
)


def layout(page_slug=None, **query_parmas):
    indicators_conf = indicators_dict  # load from DB
                                ...
                                [
                                    ...
                                    dbc.Col(
                                        dbc.DropdownMenu(
                                            ...
                                            id="aio-collapse-countries-button",
                                            className="m-2",
                                            color="info",
                                                        ...
                                                        [
                                                            ...
                                                            dbc.Popover(
                                                                [
                                                                    ...
                                                                    dbc.PopoverBody(
                                                                        id={
                                                                            "type": "aio_area_sources",
                                                                            "index": "AIO_AREA",
                                                                        }
                                                                    ),
                                                                ],
                                                                id="hover",
                                                                target="aio_area_info",
                                                                trigger="hover",
                                                            ),
                                                        ],
                                                    ),
            ...
            "graphs": {
                ...
                "map": {
                    "options": dict(
                        ...
                        geojson=get_geo_file("ecaro.geo.json"),
                        zoom=2,
...
@callback(
    ...
    Output({"type": "fig_types", "index": "AIO_AREA"}, "options"),
    Output({"type": "fig_types", "index": "AIO_AREA"}, "value"),
...
def set_aio_options(indicators_dict):
    ...
    fig_types = []
    if aio_area in indicators_dict[theme]:
        indicators = indicators_dict[theme][aio_area].get("indicators")
        area_indicators = indicators.keys() if indicators is dict else indicators

        default_option = (
            indicators_dict[theme][aio_area].get("default")
            if aio_area in indicators_dict[theme]
            else ""
        )
        ...
        fig_types = [
            ...
            for name in indicators_dict[theme][aio_area].get("graphs", {}).keys()
        ]

    default_graph = (
        indicators_dict[theme][aio_area].get("default_graph")
        if aio_area in indicators_dict[theme]
        else ""
    )

    return area_butons, fig_types, default_graph
...
@callback(
    ...
    [
        ...
        Input({"type": "fig_types", "index": "AIO_AREA"}, "value"),
    ],
    State({"type": "indicator_button", "index": ALL}, "id"),
)
...
@callback(
    ...
    [
        State({"type": "fig_types", "index": "AIO_AREA"}, "value"),
        State("indicators", "data"),
    ],
)
def set_default_compare(compare_options, selected_type, indicators_dict):
    ...
    config = indicators_dict[theme][aio_area]["graphs"][selected_type]
...
@callback(
    [
        Output({"type": "aio_area", "index": "AIO_AREA"}, "figure"),
        Output({"type": "aio_area_sources", "index": "AIO_AREA"}, "children"),
        Output("indicator_card", "children"),
    ],
    [
        Input("aio-store", "data"),
...
def aio_area_figure(
    ...
    aio_area = "AIO_AREA"
    default_graph = indicators_dict[selections["theme"]][aio_area].get(
        "default_graph", "line"
    )

    fig_type = selected_type if selected_type else default_graph
    fig_config = indicators_dict[selections["theme"]][aio_area]["graphs"][fig_type]
